...
| 现在真的做到了什么？ | 已有真实钱包入口，以及 SDK-backed create -> scan -> claim + disclosure/activity narrative 的演示闭环 |
| 当前边界是什么？ | `Disclosure` 与 `Activity` 仍是 wallet-scoped bounded summary，不应表述成完整 protocol audit / disclosure backend |
...
- Wallet-based, user-facing dApp flow with a real wallet adapter entry and an Umbra SDK-centered typed service boundary
...
- create / scan / claim 已围绕 official Umbra SDK 接通，但当前仍是 devnet-first、single-asset、happy-path-oriented 的 bounded workflow
