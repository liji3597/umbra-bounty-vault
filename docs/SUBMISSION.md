...
### 1.4 Quick Read for Judges

| Question | Short answer |
| --- | --- |
| 这是什么？ | 一个面向 bounty / grant / contributor reward 的 privacy-first reward workflow |
| 为什么是 Umbra？ | 因为核心问题不是普通支付，而是私密发放、可领取、可受控披露 |
| 现在真的做到了什么？ | 已有真实钱包入口、`Create Payout` 的 live devnet anchor，以及 create -> claim -> disclosure -> activity 的演示闭环 |
| 当前边界是什么？ | `Claim Center`、`Disclosure`、`Activity` 仍以 wallet-scoped typed demo continuity 为主，不应表述成完整 live backend |
| 评审应如何理解验证状态？ | 当前验证足以支持 bounded demo submission，但还不是完整 P7 审计收口 |
...
- Claim center for discovering and claiming a linked claimable payout
- Controlled disclosure views for verification contexts
- Activity timeline across payout, claim, and disclosure events
- Wallet-based, user-facing dApp flow with a real wallet adapter entry and typed demo-service boundaries

---

## 4.7 Demo Scope
Demo focuses on the smallest Phase 1 narrative that still reads as a credible product workflow:

- **A2-leaning, claim-oriented direction**
- **devnet-first** execution framing
- **single-asset** demo scope
- **wallet-scoped continuity** across create -> claim -> disclosure -> activity
- **live-aware** disclosure and activity surfaces that stay honest about current implementation boundaries
...
- claim, disclosure, and activity currently rely on wallet-scoped typed demo continuity rather than a full live backend or indexer stack
- the project should be described as a demo-ready workflow with a minimal live devnet anchor, not as a production-complete live treasury environment
...
- They discover the linked claimable payout in the app and claim it through the wallet-scoped reward flow shown in this demo.
...
- We also model the reward lifecycle as a coherent activity story:
  payout created, claimable state detected, claim recorded, disclosure context available.
...
- Wallet provider layer for real wallet adapter entry and network state
